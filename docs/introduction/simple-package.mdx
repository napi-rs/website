...
:::caution
It's recommend to use npm scope to name your package. Because `@napi-rs/cli` will create and publish many packages per platform for you. If these packages are not under a npm scope, it will trigger npm [**_spam detection_**](https://stackoverflow.com/a/54135900/5684750) while your are publishing them for the first time.
:::
...
Your native codes are in `src/lib.rs`. The `.cargo/config.toml` file is used in `GitHub CI` for cross compiling. In general this file is not affect your development on your local machine.
...
Let's create a `test.mjs` file to test the generated `sum` function:
...
You don't need change the `napi name` filed in the `package.json` because the package suffix is not changed. Just type **Enter** to keep the `cool` name.
