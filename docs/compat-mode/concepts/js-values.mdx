...
Represents `undefined` in JavaScript. It can not be casted into a `Rust` value, and no `Rust` value can be casted into `JsUndefined`.

The only way to create `JsUndefined` in Rust is calling `Env::get_undefined()`.

## `JsNull`

Represents `null` in JavaScript. Like `JsUndefined`, it can not be casted to a `Rust` value and no `Rust` value can be casted to it.
...
`JsBoolean` represents boolean values in JavaScript.
...
Represents a `string` value in JavaScript. There are 3 types of `string` encoding you can interactive with `N-API`: `utf8`, `utf16` and `latin1`.
...
Using `Env::create_string()` you can create `JsString` from Rust `&str`.
If you would like to get utf8 string from `JsString`, you must use the `JsString::into_utf8()` to get utf8 value explicitly.
...
Represents a `Symbol` value in JavaScript. You can create `JsSymbol` from `&str` or `JsString`
...
Represents an `Object` value in JavaScript. There are many object related API's in `JsObject`. See the [docs](https://docs.rs/napi/latest/napi/struct.JsObject.html).
...
Represents a `Date` object in JavaScript. JavaScript `Date` objects are described in [Section 20.3](https://tc39.es/ecma262/#sec-date-objects) of the ECMAScript Language Specification.

## `JsBigint`

Represents a `Bigint` value in JavaScript.
