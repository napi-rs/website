...
import { Green, Rust, Warning } from './chalk'
...
For this case, `@napi-rs/cli` provide the `--js-package-name` to override generated packages loading logic. For example in `snappy` we have <Green>package.json</Green> like this:
