...
`contextless_function` is very similar with `js_function`, except that `contextless_function` will not get the **_Context_** of this `JavaScript function`.
...
## 3. `resolve/reject` method in [Task](../../concepts/async-task#task) trait

[Task](../../concepts/async-task#task) trait is a little complicated, see it's [docs](../../concepts/async-task#task) for a dive deep into it.
