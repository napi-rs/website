...
:::caution
我们推荐将你的包发布到 npm scope 下。 因为 `@napi-rs/cli` 会在每个支持的平台下创建并发布很多个包。 如果这些包不在一个 npm scope 下，在发布的时候就会触发 npm [**_spam detection_**](https://stackoverflow.com/a/54135900/5684750)。
:::
