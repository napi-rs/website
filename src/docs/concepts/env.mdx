...
## 3. `resolve/reject` method in [Task](./concepts/task) trait

[Task](./concepts/task) trait is a litter complicated here, you may need follow the link in title to dive deep into it.
...
## 4. `ThreadSafeCallContext` in [Thread safe function](./concepts/thead-safe-function)
