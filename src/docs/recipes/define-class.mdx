...
This example creates a class named `TestClass` which has a js property called `count` and wraps a rust struct `NativeClass` on the instance of the object which has a property `value`. The methods can access both the object's properites and the wrapped struct.
