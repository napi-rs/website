---
title: 'NodeJS native module'
description: NodeJS native module.
---
...
This is where NodeJS begins, with C++ module development at its most basic. For example, we have a legitimate native module ons.node under Linux, which is actually a binary file that couldn't be seen properly in a text editor, until we came across the binary viewer.

![Hex content of native module](./hex.png)
...
This stuff It has been with Node.js since NodeJS v0.8, before that its default compilation helper package was `node-waf`(see below), which should be familiar to old Noder.
...
> As you can see from the above description, compiling C++ native modules on Windows relies on Visual Studio, which is why you will need to have Vusual Studio pre-installed to install some Node.js packages. <br/>
...
In this chapter, we introduced what the NodeJS native addon is and how to compile it. in the next chapter, we will review the history of API changes related to the native addon in NodeJS and formally introduce our protagonist: the N-API.
