...
| `packageJsonPath` | `--package-json-path` | `string` | 否 | <Green>`package.json`</Green> | 包含 napi 配置的根包。 |
| `outputDir` | `--output-dir,-o,-d` | `string` | 否 | <Green>`./artifacts`</Green> | 递归搜索所下载 `.node` 和 `.wasm` 文件的目录。它对应产物下载路径，而不是 `npm/`。 |
| `npmDir` | `--npm-dir` | `string` | 否 | <Green>`npm`</Green> | 包含生成的各平台包的目录。 |
