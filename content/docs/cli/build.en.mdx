...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--cross-compile` / `-x` | The selected helper — <Green>cargo-xwin</Green> for Windows-platform targets, <Green>cargo-zigbuild</Green> for everything else — via `cargo install`, on first use (that run can be slow). For Windows targets, cargo-xwin downloads the Microsoft CRT and Windows SDK itself (the Microsoft license applies). | `zig` on `PATH`. The CLI never installs or checks zig; cargo-zigbuild errors if it is missing.                                                                                                                                                             |
