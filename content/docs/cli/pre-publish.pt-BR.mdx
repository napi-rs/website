...
| `packageJsonPath` | `--package-json-path` | `string` | Não | <Green>`package.json`</Green> | Metadados e versão de release do pacote raiz. |
| `npmDir` | `--npm-dir,-p` | `string` | Não | <Green>`npm`</Green> | Diretório que contém um pacote preparado para cada target configurado. |
| `tagStyle` | `--tag-style,--tagstyle,-t` | `npm \| lerna` | Não | <Green>`lerna`</Green> | Forma de resolver a tag da release no GitHub. `npm` usa `v<versão do pacote>`; `lerna` lê a tag do pacote no commit de release mais recente. |
