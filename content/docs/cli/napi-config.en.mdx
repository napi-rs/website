...
| `targets`                |      <Green>`[]`</Green>      | The target triples your project ships for — used for scaffolding and packaging, not by `napi build` (see below). Target triples could be found in the output of `rustup target list` command. |
