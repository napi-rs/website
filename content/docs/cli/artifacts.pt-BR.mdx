...
| `packageJsonPath` | `--package-json-path` | `string` | Não | <Green>`package.json`</Green> | Pacote raiz que contém a configuração do napi. |
| `outputDir` | `--output-dir,-o,-d` | `string` | Não | <Green>`./artifacts`</Green> | Diretório pesquisado recursivamente em busca dos arquivos `.node` e `.wasm` baixados. Isso corresponde ao caminho de download dos artefatos, não a `npm/`. |
| `npmDir` | `--npm-dir` | `string` | Não | <Green>`npm`</Green> | Diretório que contém os pacotes gerados de cada plataforma. |
