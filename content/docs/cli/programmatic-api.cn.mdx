...
最常见的用法是对生成的 JavaScript 和 TypeScript 文件执行自定义后处理。以下示例使用 <Green>oxfmt</Green> 格式化输出文件：
...
使用与传给 <Green>napi build</Green> 相同的参数运行该脚本，包括 `--` 后面的 Cargo 参数：
...
| Kind | 描述 |
|------|-------------|
| <Green>node</Green> | 原生 Node.js 插件（<Green>.node</Green> 文件） |
| <Yellow>js</Yellow> | JavaScript 绑定文件 |
| <Blue>dts</Blue> | TypeScript 定义文件 |
| <Rust>exe</Rust> | 可执行二进制文件 |
| <Purple>wasm</Purple> | WebAssembly 模块 |
...
| dtsHeader | string | 否 | | <Green>.d.ts</Green> 文件的自定义文件头字符串；仅在两个文件头文件选项都未设置时使用 |
...
| constEnum | boolean | 否 | true | 生成 <Green>const enum</Green> 而不是普通 enum |
...
| idents | string[] | 是 | | <Green>generateTypeDef</Green> 导出的标识符 |
