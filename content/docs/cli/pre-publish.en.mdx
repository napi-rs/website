...
| `packageJsonPath` | `--package-json-path` | `string` | No | <Green>`package.json`</Green> | Root package metadata and release version. |
| `npmDir` | `--npm-dir,-p` | `string` | No | <Green>`npm`</Green> | Directory containing one prepared package per configured target. |
| `tagStyle` | `--tag-style,--tagstyle,-t` | `npm \| lerna` | No | <Green>`lerna`</Green> | How to resolve the GitHub release tag. `npm` uses `v<package version>`; `lerna` reads the package tag from the latest release commit. |
