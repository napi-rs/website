...
The Windows branch routes by the target's *platform*, so `x86_64-pc-windows-gnu` lands in the xwin branch too — but cargo-xwin is MSVC-only, so `-x` does not actually work for that triple. Prefer the `*-pc-windows-msvc` triples, and if you really need windows-gnu, build it without a cross flag — see the windows-gnu note in [Recipes per target](#recipes-per-target).
