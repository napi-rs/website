...
The Windows branch routes by the target's *platform*, so it covers `x86_64-pc-windows-gnu` too — but as a matter of policy, prefer the `*-pc-windows-msvc` triples unless you specifically target MSYS2/MINGW Node; see the windows-gnu note in [Recipes per target](#recipes-per-target).
