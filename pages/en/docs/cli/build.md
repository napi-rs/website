...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--cross-compile` / `-x` | The selected helper — <span class="chalk-green">cargo-xwin</span> for Windows-platform targets, <span class="chalk-green">cargo-zigbuild</span> for everything else — via `cargo install`, on first use (that run can be slow). For Windows targets, cargo-xwin downloads the Microsoft CRT and Windows SDK itself (the Microsoft license applies). | `zig` on `PATH`. The CLI never installs or checks zig; cargo-zigbuild errors if it is missing.                                                                                                                                                            |
...
`--cross-compile` picks cargo-xwin by the target's **platform**: every `*-windows-*` triple built from a non-Windows host routes through cargo-xwin — including `x86_64-pc-windows-gnu`, not just MSVC. Every non-Windows target goes through cargo-zigbuild — there is no allowlist and no host == target check.
