...
| crossCompile      | --cross-compile,-x    | boolean  | false    |         | [experimental] cross compile by swapping the cargo subcommand: Windows MSVC targets from a non-Windows host use <span class="chalk-green">cargo-xwin</span>; every other target uses <span class="chalk-green">cargo-zigbuild</span> (requires <span class="chalk-green">zig</span> on PATH). Both subcommands are auto-installed. Conflicts with <span class="chalk-green">--use-cross</span>                                                                  |
...
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--use-cross` + `--cross-compile`      | Hard error: the CLI refuses to build (possibly after auto-installing cargo-xwin and cargo-zigbuild first).                                                                     |
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--cross-compile` / `-x` | <span class="chalk-green">cargo-xwin</span> and <span class="chalk-green">cargo-zigbuild</span>, via `cargo install` (the first run can be slow). For Windows targets, cargo-xwin downloads the Microsoft CRT and Windows SDK itself (the Microsoft license applies). | `zig` on `PATH`. The CLI never installs or checks zig; cargo-zigbuild errors if it is missing.                                                                                                                                                            |
...
| `--use-napi-cross`       | The gcc toolchain, downloaded automatically from npm (<span class="chalk-green">@napi-rs/cross-toolchain</span>) and cached under `~/.napi-rs/cross-toolchain`.                                                                                                       | `npm` on `PATH`, and a Linux x64 or arm64 host. The CLI does not check the host: on macOS or Windows the download "succeeds" and the Linux gcc fails at link time. Any failure here only prints a warning and the build continues as plain `cargo build`. |
...
`--cross-compile` picks cargo-xwin per **target** (a Windows MSVC target built from a non-Windows host), not per host. Every non-Windows target goes through cargo-zigbuild — there is no allowlist and no host == target check.
...
Android, WASI and OpenHarmony targets always get their toolchain env from `ANDROID_NDK_LATEST_HOME`, `WASI_SDK_PATH` and `OHOS_SDK_PATH` respectively — with or without any cross flag.
