...
| `dtsHeader`               |  <span class="chalk-green">`undefined`</span>  | String used as the header of the generated declaration file. It replaces the default header entirely; see [Declaration file header](#declaration-file-header).                                                                |
| `dtsHeaderFile`           |  <span class="chalk-green">`undefined`</span>  | Path, relative to the command's working directory, to a file whose content is used as the declaration file header. It takes precedence over `dtsHeader`.                                                                      |
...
You can replace this header with your own TypeScript types, imports, license
comments, or lint directives. A custom header **replaces** the default header
entirely; include the auto-generated comment and the eslint directive in your
...
| `--dts-header`    | CLI flag of [`napi build`](./build) | CI/CD overrides              |
| `--no-dts-header` | CLI flag of [`napi build`](./build) | Disable the header entirely  |
...
`--no-dts-header` skips header resolution and generates the `.d.ts` without
any header.
...
See [Types overwrite](/docs/concepts/types-overwrite) for the attribute-level
TypeScript overrides (`ts_args_type`, `ts_return_type`, and friends), which
change individual declarations rather than the file header.
