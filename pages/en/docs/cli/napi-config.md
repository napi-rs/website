...
| `targets`                |      <span class="chalk-green">`[]`</span>      | The target triples your project ships for — used for scaffolding and packaging, not by `napi build` (see below). Target triples could be found in the output of `rustup target list` command. |
...
invocation, selected with its `--target` flag. The cross-compilation flags
