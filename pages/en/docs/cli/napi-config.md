...
| `targets`                |      <span class="chalk-green">`[]`</span>      | The target triples your project ships for — drives scaffolding and packaging; not a multi-target build switch, though `napi build` does read it to generate the WASI bindings (see below). Target triples could be found in the output of `rustup target list` command. |
...
binding files (`<binaryName>.wasi.cjs` and friends) entirely when no WASI
