...
Your native code is in `src/lib.rs`. The `.cargo/config.toml` file carries the per-target linker configuration that cross-compiling in the `GitHub CI` depends on. In general, this file does not affect your development on your local machine, but don't delete it: without it, cross targets fail with confusing link errors — and a `zig: not found` line in the build log is a red herring, not the real problem. See the [Cross build](/docs/cross-build) guide for how cross-compilation works.
