...
Your native code is in `src/lib.rs`. The `.cargo/config.toml` file currently only sets `rustflags` that statically link the C runtime for Windows MSVC targets — don't delete it even though it looks trivial, or your Windows binaries will start depending on the MSVC runtime DLLs. Cross-compile linker and toolchain configuration does not live in this file: it comes from the `napi` CLI itself (see [napi build](/docs/cli/build)). In general, this file does not affect your development on your local machine. See the [Cross build](/docs/cross-build) guide for how cross-compilation works.
