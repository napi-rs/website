...
Your native code is in `src/lib.rs`. The `.cargo/config.toml` file currently only sets `rustflags` that statically link the C runtime for some Windows MSVC targets — `x86_64-pc-windows-msvc` in this template (the pnpm template also covers `i686`) — don't delete it even though it looks trivial, or those Windows builds will start depending on the MSVC runtime DLLs. Cross-compile linker and toolchain configuration does not live in this file: it comes from the `napi` CLI itself (see [napi build](/docs/cli/build)). In general, this file does not affect your development on your local machine. See the [Cross build](/docs/cross-build) guide for how cross-compilation works.
