...
The generated CI workflow is the template's workflow filtered down to the platforms you pick, so a build job appears only for targets the template ships a matrix row for: macOS (x64, arm64), Windows MSVC (x64, x86, arm64), Linux gnu and musl (x64, arm64), armv7 gnueabihf, Android (arm64, armv7), WASI and FreeBSD — most of them built by cross-compiling rather than on native hardware. The other accepted targets (OpenHarmony, `x86_64-pc-windows-gnu`, `armv7-unknown-linux-musleabihf`, loongarch64, riscv64gc, ppc64le, s390x) can be selected at the prompt, but `napi new` only ever filters the template — it never adds to it — so they end up in neither the generated workflow nor the scaffolded `napi.targets` list that `napi create-npm-dirs` and the publish flow read: no CI build job, no npm package. To use one, add it to `targets` in the napi config, run `napi create-npm-dirs`, and add the CI matrix row yourself — see [Add a target to an existing project](/docs/cross-build#add-a-target-to-an-existing-project) for the full recipe. The [Cross build](/docs/cross-build) guide also explains the cross-compile flags the generated CI uses.
...
- `@cool/core` includes just `JavaScript` code, which actually loads the native binary from each platform.
...
In each platform binary package, there are `cpu` and `os` fields in the `package.json`:
...
5. Run `yarn napi rename` command under the project folder to rename your package.

### `pnpm`

1. Go to [GitHub template project](https://github.com/napi-rs/package-template-pnpm)
2. **Click Use this template**.
3. Clone your project.
4. Run `pnpm install` to install dependencies.
5. Run `pnpx napi rename` command under the project folder to rename your package.
