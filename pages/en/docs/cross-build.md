...
- **`--cross-compile`** (**`-x`**) for Windows targets from a non-Windows host (`cargo-xwin` under the hood), and for musl — or glibc, macOS and FreeBSD targets when the options above are not available on your host — via `cargo-zigbuild`.
