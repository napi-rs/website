...
We also integrate the `cargo-zigbuild` and `cargo-xwin` in the `@napi-rs/cli`, so you can build many different targets on a single machine, see [**Cross Compilation**](/docs/cross-build/summary) for more details.
...
<a href="https://deno.com/"><DenoLogo /></a> and <a href="https://bun.sh/"><img src={bunLogo.src} style={{ verticalAlign: 'text-bottom' }} width={20} height={20} /> Bun</a> has been improved their <NodeLink href="https://nodejs.org/api/n-api.html"> **Node-API** </NodeLink> compatibility, almost all the **NAPI-RS** projects can run in **Deno** and **Bun**.
