...
We also integrate the `cargo-zigbuild` and `cargo-xwin` in the `@napi-rs/cli`, so you can build many different targets on a single machine, see [**Cross Compilation**](/docs/cross-build) for more details.
