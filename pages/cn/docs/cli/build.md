...
| `--use-napi-cross` + `--cross-compile` | 打印警告说 `--cross-compile` 将被忽略 —— 但 `cargo zigbuild` 仍然会运行，同时 napi-cross 的环境变量也被设置（napi-cross 的 linker 会压过 zig）。不要组合使用。 |
...
- **Android**：linker/CC/AR 环境变量由 `ANDROID_NDK_LATEST_HOME` 构造。变量缺失只会打印警告 —— 环境变量仍会被设置成坏掉的路径，构建在链接阶段失败。当宿主机本身就是 Android 时，整套设置会被跳过。
