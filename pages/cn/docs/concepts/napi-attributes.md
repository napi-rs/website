...
如果初始化不需要 exports 对象，请参阅[模块初始化](/docs/concepts/module-init)。
...
明确不保证每个 panic 都可以展开。预期内的失败应使用 `Result`。参阅[错误处理](/docs/concepts/error-handling)。
...
`iterator` 与 `async_iterator` 互斥。生成器类不能暴露名为 `next`、`return` 或 `throw` 的公开字段，因为 napi-rs 会安装这些协议方法。有关所需 trait 和生命周期约束，请参阅[迭代器与异步迭代器](/docs/concepts/iterators)。
