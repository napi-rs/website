...
在 Node.js 中，[`Threadsafe Function`](https://nodejs.org/api/n-api.html#asynchronous-thread-safe-function-calls) 是一个复杂的概念。众所周知，Node.js 是单线程的，因此不能从其他线程访问 [`napi_env`](https://nodejs.org/api/n-api.html#napi_env)、[`napi_value`](https://nodejs.org/api/n-api.html#napi_value) 和 [`napi_ref`](https://nodejs.org/api/n-api.html#napi_ref)。
...
默认情况下，`ThreadsafeFunction` 会让创建它的线程上的 event loop 保持存活，直至 `ThreadsafeFunction` 被销毁。参阅 [**决定是否让进程保持运行**](https://nodejs.org/api/n-api.html#deciding-whether-to-keep-the-process-running)。
...
`Status::QueueFull`。更多细节请参阅 [`napi_call_threadsafe_function`](https://nodejs.org/api/n-api.html#napi_call_threadsafe_function)。
